...
// A Word Ladder is a sequence of dictionary words, where each word in the sequence differs from the previous word by exactly one letter. Given two starting words, determine the length of the shortest word ladder that connects the two. Each word in the sequence must be in the English dictionary.
...
public class WordLadder {
  ...
  // start word, end word, disctionary(all words), return length
  // all uppercase
   ...
   */
  public static List<String> wordLadder(String start, String target, Set<String> wordDict) {
    // BFS
    Queue<WordObj> wordQueue = new LinkedList<WordObj>();
    WordObj word = new WordObj(start, Arrays.asList(start)); // start with first word
    wordQueue.add(word);
  ...
  public static void main(String[] args) {
    ...
    // int ladderLength2 = wordLadder(start1, end1, englishDictionary);
